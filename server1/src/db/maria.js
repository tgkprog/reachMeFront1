...
function initMaria() {
  ...
  pool = mysql.createPool({
    ...
    database: process.env.DB_NAME || "reachm1",
...
function getPool() {
  if (!pool) return initMaria();
  return pool;
}

// Generic execute compatible with mysql2: returns [rows, fields]
...
// Public ReachMe and messages API
async function getPublicReachMeCounts(userId) {
  const [[{ totalCount }]] = await execute(
...
async function getPublicReachMeByCode(urlCode) {
  const [rows] = await execute(
    `SELECT p.id, p.user_id, p.is_active, p.deactivate_at, u.email as user_email
       FROM pblcRechms p
       JOIN users u ON p.user_id = u.id
       WHERE p.url_code = ? LIMIT 1`,
    [urlCode]
  );
  return rows[0] || null;
}
...
async function deleteUserCascade(userId) {
  // try to fetch user email for invites cleanup
  ...
  // remove invites by email if we have it
  if (email) {
