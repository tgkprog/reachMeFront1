...
const AlarmsScreen: React.FC<any> = ({navigation}) => {
  ...
  const load = async () => {
    ...
    if (Platform.OS === 'web' && 'Notification' in window) {
      setNotificationsBlocked(Notification.permission !== 'granted');
    }
  };
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <Text style={styles.meta}>Stored max 100, auto-removed after 2 days.</Text>
      ...
      {alarms.length === 0 && <Text style={styles.empty}>No alarms received yet.</Text>}
      {alarms.map(a => (
...
const styles = StyleSheet.create({
  ...
  content: {padding: 16},
  header: {fontSize: 22, fontWeight: '600', marginBottom: 4, textAlign: 'center'},
  meta: {fontSize: 12, color: '#666', marginBottom: 12, textAlign: 'center'},
  banner: {backgroundColor: '#ffe9d5', padding: 12, borderRadius: 8, marginBottom: 16},
